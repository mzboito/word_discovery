...
col = ["epoch",'loss', 'nll_loss', 'ppl', 'wps', 'ups', 'wpb', 'bsz', 'num_updates', 'lr', 'gnorm', 'clip', 'oom', 'wall', 'train_wall', "valid on 'valid' subset", 'valid_loss', 'valid_nll_loss', 'valid_ppl', 'num_updates', 'best']
    ...
    with codecs.open(f_path, "r","utf-8") as input_file:
        for line in input_file:
            if start:
                ...
                try:
                    dictionary[key] += clean_line(line[2:])
                except KeyError:
                    dictionary[key] = clean_line(line[2:])
...
def write_table(d, output_file):
    with open(output_file,"w") as output_file:
        ...
        for key in d.keys():
            output_file.write("\t".join([key] + d[key]) + "\n")
