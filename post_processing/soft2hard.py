...
def types_entropy(f_path, dictionary):
    ...
    for i in range(1, len(matrix)): #for each line
        ...
        if last_col: #first element
            ...
            discovered_tokens.append(finalString)
            finalString = matrix[i][0]
            index_list.append([i])

    print(discovered_tokens)
    print(index_list)
    print(segment_target(matrix, True))
    exit(1)

    return dictionary

def write_dictionary(f_path):
    pass
...
def run(args):
    ...
    if args.type_entropy:
        ...
        write_dictionary(output_path + "/types_entropy.csv")
